...
class mata_generater(nn.Module):
    def __init__(self,
        patch_size=8,
        pooling_size=2, #定义每次下采样的池化size
        in_channel_nums=3,
    ):
        super(mata_generater,self).__init__()
    ...
    def forward(self,x,adv,pgd,shuffle):
        ...
        map_k=F.interpolate(input=map_k,scale_factor=self.patch_s,mode="nearest")
        map_k=torch.where(map_k==1,pgd,map_k)
        map_k=torch.where(map_k==2,adv,map_k)
        map_k=torch.where(map_k==3,shuffle,map_k)
